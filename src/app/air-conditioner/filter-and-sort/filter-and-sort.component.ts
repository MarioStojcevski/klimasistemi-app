...
export class FilterAndSortComponent implements OnInit {
  ...
  private getAllBrands(): void {
    this.airConditionerService.getAllBrands().subscribe((brands) => {
  ...
  public filter(): void {
    let unFormattedPowerArray: string[] =
      this.filterForm.get('powerArrayControl')?.value;
    let formattedPowerArray: number[] = unFormattedPowerArray.map(
      (x) => +x * 1000
    );
    ...
    this.filterDto = {
      ...
      filterByBrand:
        this.filterForm.get('filterByBrandControl')?.value !== undefined
          ? this.filterForm.get('filterByBrandControl')?.value
          : null,
    };

    if (this.filterDto.maxPrice - this.filterDto.minPrice > 0) {
      this.airConditionerService
        .getAllAirConditionersFiltered(this.filterDto)
        .subscribe(
          (result) => {
